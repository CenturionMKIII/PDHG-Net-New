...
def train_data(train_num, num_nodes, degree, damping_factor, train=True):
...
def gen_files(n_files, num_nodes, degree, damping_factor):
    if not os.path.isdir('../data/train'):
        os.mkdir('../data/train')
    if not os.path.isdir('../data/valid'):
        os.mkdir('../data/valid')
    for i in range(n_files[0]):
        train_data(i, num_nodes, degree, damping_factor, train=True)
